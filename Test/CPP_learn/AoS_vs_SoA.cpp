...
const long long ELEMS = 10'000'000;

// TODO : make sure whole project notice this performance problem
...
uint64_t averageNameLen(const VP &v)
{
    return accumulate(begin(v), end(v), (uint64_t)0,
                      [](auto sum, auto &p) { return sum + p.name.length(); }) /
           v.size();
}
...
{
    return accumulate(begin(v), end(v), (uint64_t)0,
                      [](auto sum, auto &p) { return sum + p.age; }) /
           v.size();
}
...
{
    return accumulate(begin(v), end(v), (uint64_t)0,
                      [](auto sum, auto &p) { return sum + p.dob; }) /
           v.size();
}
...
struct Persons
{
    ...
    uint64_t averageNameLen() const
    {
        return accumulate(begin(names), end(names), (uint64_t)0,
                          [](auto sum, auto &n) { return sum + n.length(); }) /
               names.size();
    }
