...
namespace Rocket {
    ...
    //---------------------------------------------------------------------------------------------------------------------
    bool Process::AttachToParent(StrongProcessPtr pParent)
    {
        ...
        {
            RK_CORE_WARN("Attempting to attach a dead process to a parent");
            return false;
        }
        else if (IsRemoved())
        {
            RK_CORE_WARN("Attempting to attach a process that is already being removed to another parent");
            return false;
        }

        // Attach this process to the new parent.  It will be attached at the very end of the child tree.
        //pParent->AttachChild(std::make_shared(this));
